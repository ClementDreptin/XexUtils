...
void Math()
{
    Describe("Math::Radians(float)");

    It("converts degrees to radians", []() {
        TEST_BETWEEN(Math::Radians(45.0f), 0.78f, 0.79f);
    });

    Describe("Math::ProjectForward(const Math::vec3 &, float, float)");
    ...
    Describe("Math::RoundToNextMultiple<N>(uint32_t)");

    It("rounds to the next multiple of N", []() {
